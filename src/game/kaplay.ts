...
export function getKaplayInstance() {
  return _instance
}
...
export function loadAllAssets(k: ReturnType<typeof kaplay>) {
  ...
  k.loadSpriteAtlas('/assets/tilesets/TilesetElement.png', {
    ...
    'big-signboard': { x: 176, y: 16, width: TILE_SIZE * 3, height: TILE_SIZE },
  })
...
export const ZONE_COLORS: Record<number, [number, number, number]> = {
  1: [100, 200, 100] as [number, number, number],   // green
  2: [100, 150, 220] as [number, number, number],   // blue
  3: [180, 80, 180] as [number, number, number],   // purple
} as const
